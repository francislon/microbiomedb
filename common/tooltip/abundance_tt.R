get_sample_from_action <- function(ggplot_object, action){
  gg_bld_obj<-ggplot_build(ggplot_object)
  x_is_disc <- gg_bld_obj$layout$panel_scales$x[[1]]$is_discrete()
  y_is_disc <- gg_bld_obj$layout$panel_scales$y[[1]]$is_discrete()
  ...
  if(nrow(gg_bld_obj$layout$panel_layout)==1){
    ...
    panel_layout<-gg_bld_obj$layout$panel_layout # panel_layout returns a data.frame
    ...
    if(is.null(action$panelvar1)){
      # panel_hover<-panel_layout[is.na(panel_layout[[facet_column]]),]
      panel_hover <- subset(panel_layout, is.na(get(facet_column))) 
      
    }else{
      panel_hover <- subset(panel_layout, get(facet_column)==action$panelvar1) 
    }
    panel_index <- panel_hover$PANEL
  }
  sample<-gg_bld_obj$layout$panel_ranges[[panel_index]]$y.labels[x_]
  sample
}
